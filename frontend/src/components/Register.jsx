  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      Swal.fire({
        icon: 'error',
        title: 'Erreur',
        text: 'Le mot de passe doit contenir au moins 8 caracteres, une majuscule, une minuscule et un chiffre.',
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginBottom: 6 }}>
                Numero de telephone <span style={{ color: '#ccc', fontWeight: 400 }}>(optionnel)</span>
              </label>
            ...
            <div style={{ marginBottom: 28 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginBottom: 6 }}>Confirmer le mot de passe</label>
              <div style={{ position: 'relative' }}>
                <input type={showPwd2 ? 'text' : 'password'} value={password2} onChange={e => setPassword2(e.target.value)}
                  placeholder="••••••••" required style={{ ...inputStyle, paddingRight: 42,
                    borderColor: password2 && password !== password2 ? '#ef4444' : '#e5e7eb',
                  }} />
